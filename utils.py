...
def sex_vendor(vendor_sex_in_main):
    vendor_sex = ""
    if vendor_sex_in_main == "Случайно":
        sex = ["Мужчина", "Женщина"]
        vendor_sex = random.choice(sex)
    else:
        vendor_sex = vendor_sex_in_main
    return vendor_sex


def name_vendor(vendor_sex, vendor_name_in_main):
    vendor_name = ""
    if vendor_name_in_main == "":
        if vendor_sex == "Мужчина":
            vendor_name = random.choice(name_man) + " " + random.choice(family)
        else:
            vendor_name = random.choice(name_woman) + " " + random.choice(family)
    else:
        vendor_name = vendor_name_in_main
    return vendor_name


def age_vendor(age_vendor_in_main):
    vendor_age = ""
    if age_vendor_in_main == "Случайно":
        age = ["Молодой", "Средний", "Пожилой"]
        vendor_age = random.choice(age)
    else:
        vendor_age = age_vendor_in_main
    return vendor_age


def race_vendor(race_vendor_in_main):
    vendor_race = ""
    if race_vendor_in_main == "Случайно":
        ...
        vendor_race = race_vendor_in_main
    return vendor_race


def money_vendor(vendor_money_in_main):
    vendor_money = 0
...
def assortment_store(shop_type, shop_cost):
    '''
